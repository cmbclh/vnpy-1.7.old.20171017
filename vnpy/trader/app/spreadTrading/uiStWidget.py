...
from vnpy.trader.app.spreadTrading.stAlgo import StAlgoTemplate
from vnpy.trader.app.spreadTrading.stBase import (EVENT_SPREADTRADING_TICK, EVENT_SPREADTRADING_POS,
                                                  EVENT_SPREADTRADING_LOG, EVENT_SPREADTRADING_ALGOLOG)
...
import sys
sys.path.append('../')
sys.path.append('D:/tr/vnpy-1.7/vnpy')
import vnpy.common
from vnpy.common import *
...
class StPosMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        d['actlegLongPos'] = {'chinese':u'主动腿多头持仓', 'cellType':BasicCell}
        d['actlegLongValue'] = {'chinese':u'主动腿多头成本', 'cellType':PnlCell}
        d['actlegShortPos'] = {'chinese':u'主动腿空头持仓', 'cellType':BasicCell}
        d['actlegShortValue'] = {'chinese':u'主动腿空头成本', 'cellType':PnlCell}

        d['passlegLongPos'] = {'chinese':u'被动腿多头持仓', 'cellType':BasicCell}
        d['passlegLongValue'] = {'chinese':u'被动腿多头成本', 'cellType':PnlCell}
        d['passlegShortgPos'] = {'chinese':u'被动腿空头持仓', 'cellType':BasicCell}
        d['passlegShortValue'] = {'chinese':u'被动腿空头成本', 'cellType':PnlCell}

        d['profitloss'] = {'chinese':u'浮动盈亏', 'cellType':PnlCell}
        self.setHeaderDict(d)
        ...
        print(u'持仓监控表头列表%s' % str(len(self.headerDict)))
        #vnpy.common.logger(u'持仓监控表头列表%s' % str(len(self.headerDict)))
        #self.writeLog(u'持仓监控表头列表2%s' % str(len(self.headerDict)))
        #for i,hd in self.headerDict:
        #    print u'当前元素：',self.headerDict[i]
        #   common.writeLog(u"当前元素：%s" % hd)
        #   self.writeLog(u'持仓监控表头列表%s' % str(len(self.headerDict)))

        #common.logger.info(u"持仓监控表头列表：%s" %  str(len(self.headerDict)))
        self.initTable()
        #common.logger.info(u"持仓监控表头initTable：%s" % str(len(self.headerDict)))
        self.registerEvent()        
...
class StManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        ...
        grid.addWidget(groupTick, 1, 0, 1, 2)
        grid.addWidget(groupPos, 2, 0, 1, 2)
        grid.addWidget(groupAlgo, 3, 0, 1, 2)
        grid.addWidget(groupLog, 4, 0)
        grid.addWidget(groupAlgoLog, 4, 1)
